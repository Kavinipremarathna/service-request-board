...
import { createJobSchema, CATEGORIES, type CreateJobFormValues } from '@/lib/validations';
...
export default function NewJobPage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<CreateJobFormValues>({
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Post a Service Request</h1>
        <p className="mt-1 text-sm text-slate-500">
          Qualified tradespeople will be able to find and respond to your request.
        </p>
      </div>
      ...
      >
        <div>
          ...
          <input {...register('title')} placeholder="e.g. Leaking kitchen tap needs urgent repair" className={inputClass} />
          <FieldError message={errors.title?.message} />
        </div>

        <div>
          <label className={labelClass}>Description <span className="text-red-400">*</span></label>
          <textarea {...register('description')} rows={4} placeholder="Describe the work needed, any access requirements, urgency..." className={`${inputClass} resize-none`} />
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <div>
            ...
            <input {...register('location')} placeholder="e.g. Glasgow" className={inputClass} />
            <FieldError message={errors.location?.message} />
          </div>
        </div>

        <div className="border-t border-slate-100 pt-1">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Contact Details</p>
        </div>

        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            <label className={labelClass}>Your Name <span className="text-red-400">*</span></label>
            <input {...register('contactName')} placeholder="Full name" className={inputClass} />
            <FieldError message={errors.contactName?.message} />
          </div>
          <div>
            <label className={labelClass}>Email Address <span className="text-red-400">*</span></label>
            <input {...register('contactEmail')} type="email" placeholder="you@example.com" className={inputClass} />
        ...
        <div className="pt-2">
          ...
          >
            {isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" />Posting...</> : 'Post Job Request'}
          </motion.button>
        </div>
      </motion.form>
    </div>
