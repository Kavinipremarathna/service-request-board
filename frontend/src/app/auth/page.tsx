...
const registerSchema = z.object({
  ...
  role: z.enum(['homeowner', 'worker'], { errorMap: () => ({ message: 'Please choose a role' }) }),
});
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { value: 'homeowner', label: 'Homeowner', desc: 'Post & manage jobs', Icon: Home },
            { value: 'worker', label: 'Tradesperson', desc: 'Browse & work jobs', Icon: HardHat },
            ...
            >
              ...
              <span className={`text-xs ${selectedRole === value ? 'text-slate-300' : 'text-slate-400'}`}>{desc}</span>
            </button>
          ))}
        </div>
    ...
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center px-4 py-12">
      ...
      >
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">TradeBoard</h1>
          <p className="mt-1 text-sm text-slate-500">Connect homeowners with skilled tradespeople</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-8 shadow-sm">
          ...
          <p className="mt-5 text-center text-xs text-slate-400">
            {tab === 'login' ? (
              <>No account?{' '}
                <button onClick={() => setTab('register')} className="font-medium text-slate-700 hover:underline">
                  Create one
                </button>
              </>
            ) : (
              <>Already have an account?{' '}
                <button onClick={() => setTab('login')} className="font-medium text-slate-700 hover:underline">
                  Sign in
                </button>
              </>
            )}
